...
crazy_arrays = [
    ...
    #([0, 1, 2], np.array([np.array([0, 1], dtype=object), 2])), #can be made to work by putting dtype=model.dtype in the np.array() in the try block?!
...
class FlattenTest(BaseTest):
    ...
    #     self.assertTrue(self.mixed_iterable_equal(flatten(a), r))#flatten() returns a generator here and this is what we want
    #     self.assertTrue(self.mixed_iterable_equal(unflatten(r, a), a))
